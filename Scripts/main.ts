class SkillsListManager {
  ...
  constructor(
    ...
    /*getSkillInputs(): NodeListOf<HTMLInputElement>,*/ addButton: HTMLButtonElement,
    maxAdds: number
  ) {
  ...
  private eventInitialize(): void {
    this.inputAddButton.addEventListener("click", () => {
      this.addSkillsInputs();
    });
    this.skillsInputList.addEventListener("keydown", (event) => {
      this.backspaceInputsRemove(event);
    });
  }
...
const skillsInputList = document.querySelector(
  "#skills-input-list"
) as HTMLOListElement;
// const skillsInputs = document.querySelector('.skill-inputs') as NodeListOf<HTMLInputElement>;
const inputsAddButton = document.querySelector(
  "#add-skill-inputs-button"
) as HTMLButtonElement;
const maxInputsAddition: number = 5;

document.addEventListener("DOMContentLoaded", () => {
  const skillsListManager = new SkillsListManager(
    skillsInputList,
